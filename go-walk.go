...
func getFieldList(l *ast.FieldList) string {
    if l != nil {
        fields := []string{}
        for _, f := range l.List {
            fields = append(fields, getNames(f.Names) + " " + fmt.Sprintf("%v", f.Type))
        }
...
func (w GoWalker) Visit(node ast.Node) (ret ast.Visitor) {
    ...
    case *ast.FuncDecl:
        fmt.Println("//-")
        fmt.Println("//- func", getFieldList(n.Recv), n.Name, "(", getFieldList(n.Type.Params), ")", getFieldList(n.Type.Results))
        w.Visit(n.Body)
